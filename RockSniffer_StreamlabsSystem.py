...
#---------------------------------------
ScriptName = "RockSniffer"
Website = "https://github.com/kokolihapihvi/RockSniffer"
...
WinnerFile = os.path.join(os.path.dirname(__file__), "gg_winner.txt") ## SLCustoms section ##
...
def Execute(data):
	...
	if data.IsChatMessage() and data.IsFromTwitch():
		if data.GetParam(0).lower() == Settings.gg_start_command:
			...
			threading.Thread(target=EndGame, args=(Guess)).start()
			...
			if m_GuessingGame.AddGuess(data.User, Guess):
				Parent.SendTwitchMessage("@{0} guesses {1:.2f}%. Thanks for playing!".format(data.User, Guess))
	...
	if Settings.gg_autoend:
		...
		if not m_GuessingGame is None and not m_GuessingGame.Completed:
			...
			if m_Sniffer.GetState() == SnifferState.IN_MENUS or m_Sniffer.GetState() == SnifferState.SONG_ENDING:
				threading.Thread(target=EndGame, args=(m_Sniffer.GetAccuracy())).start()
...
def StartGame():
	...
	Parent.SendTwitchMessage("Step right up!! You have {0} seconds to !guess Boom's accuracy and win {1} {2}".format(Settings.gg_closedelay, Settings.gg_reward, Parent.GetCurrencyName()))
...
def CloseGame():
	...
	Parent.SendTwitchMessage("The guessing game is now closed, I can't wait to see who wins!")
...
def EndGame(accuracy):
	...
	if len(Winners) == 0:
		Parent.SendTwitchMessage("The guessing game has ended. Nobody guessed, but that's all good. We just chillin tonight.")
		return
	...
	if Settings.gg_jackpot:
		if Winners[0]["distance"] <= Settings.gg_jackpot_threshold:
			Parent.SendTwitchMessage("Boom's accuracy is {0:.2f}%, {1} hit the JACKPOT and wins {2} {3}. That's So Much winning!!!".format(accuracy, my_join(Winner_Names), Settings.gg_jackpot_reward, Parent.GetCurrencyName()))
	...
	Parent.SendTwitchMessage("Boom's accuracy is {0:.2f}%, {1} had the closest guess and wins {2} {3}. YAY!!!".format(accuracy, my_join(Winner_Names), Settings.gg_reward, Parent.GetCurrencyName()))
